...
Manager::Manager(const pcbManager &pcbM, const rcbManager &rcbM):m_pmanager(pcbM), m_rmanager(rcbM){
	m_register["init"] = &Manager::init;
	m_register["cr"] = &Manager::creat;
	m_register["de"] = &Manager::deletethem;
	m_register["req"] = &Manager::request;
	m_register["rel"] = &Manager::release;
	m_register["to"] = &Manager::timeOut;
	m_register["list"] = &Manager::list;
	m_register["cur"] = &Manager::curProcess;
	m_register["pr"] = &Manager::printPCB;
}

string
Manager::timeOut(const vector<string> &strs){
...
void
Manager::scheduler(int operationInformation) {

	PCB * p = findHighestPriorityProcess();

	if (operationInformation == POSSIBLE_RACE_CONTROL)
	{
		...
		else if (p->m_priority > curprocess->m_priority)
		{
			...
			operationInformation = LET_NEXT_PROCESS_RUN;
		}
	}

	if (operationInformation == LET_NEXT_PROCESS_RUN)
	{
			//把p从原在的ready 队列中删除
	...
	if (operationInformation == RACE_TO_CONTROL)
	{
		//把当前进程设为ready，并添加到对应优先级的队列中去
	...
	if (operationInformation == TIME_OUT)
	{
		PCB *cur = m_pmanager.m_runningProcess;
...
string
Manager::init(const vector<string>& strs)
{
...
string
Manager::release(const vector<string> &strs) {
	if (strs.size() != 3)
		return "Wrong input!";
	...
	if (status == RELEASE_SUCCESS)
	{
			...
			m_rmanager.m_waitingQueue[releasedResourceID - 1][0].num)//下一个进程获得资源
		{
			...
			scheduler(POSSIBLE_RACE_CONTROL);
		}
...
string
Manager::deletethem(const vector<string> &strs) {
	...
	if (p == currunningProcess)
	{
		...
		scheduler(LET_NEXT_PROCESS_RUN);

	}
	else 	scheduler(POSSIBLE_RACE_CONTROL);;//释放的进程可能抢占当前进程，也可能不抢占
	string retVal = "";
...
string 
Manager::request(const vector<string> &strs) {
...
string
Manager::list(const vector<string> &strs) {
	if (strs.size() != 2)return "Wrong input!";
	else if (strs[1] == "ready") {
		return listReady(strs);
	}
	else if (strs[1] == "block") {
		return listBlock(strs);
	}
	else if (strs[1] == "res") {
		return listAvailableResources(strs);
	}
...
string
Manager::listReady(const vector<string> &strs) {
...
string
Manager::listBlock(const vector<string> &strs) {
...
string
Manager::listAvailableResources(const vector<string> &strs) {
...
string
Manager::curProcess(const vector<string> &strs) {
	PCB * p = m_pmanager.m_runningProcess;
...
string
Manager::creat(const vector<string>& strs) {
	if (strs.size() != 3)
...
string
Manager::interface(CommandType command, const vector<string> &strs){
    int status = 0;
	string retVal;
	retVal = (this->*command)(strs);
	return retVal;
}
...
CommandType
Manager::getCom(const string& str) {
	if (!str.size())return NULL;
	else if (m_register.count(str) > 0)
	{
		return m_register[str];
	}
}
